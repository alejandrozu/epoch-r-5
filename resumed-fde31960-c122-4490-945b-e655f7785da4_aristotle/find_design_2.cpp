...
int main() {
    ...
    // A[i][j] = number of times an r-subset in orbit j is covered by q-orbit i
    vector<vector<int>> A(q_orbit_list.size(), vector<int>(r_orbit_list.size(), 0));

    // To compute A[i][j], we can just pick ONE representative of r-orbit j
    // and count how many q-subsets in q-orbit i cover it.
    
    vector<uint64_t> r_reps = r_orbit_list;
    for (int i = 0; i < q_orbit_list.size(); ++i) {
        ...
        for (uint64_t q_sub : q_orbit_members) {
            for (int j = 0; j < r_reps.size(); ++j) {
                if ((q_sub & r_reps[j]) == r_reps[j]) {
                    A[i][j]++;
                }
            }
        }
